...
if($subject)
{
  $r=mysqli_query($h,"SELECT * FROM filepdf WHERE userid='$id' AND subject='$subject' ORDER BY date_time DESC LIMIT {$_POST['limit']} OFFSET {$_POST['start']}");
}else{
$r=mysqli_query($h,"SELECT * FROM filepdf WHERE userid='$id' ORDER BY date_time DESC LIMIT {$_POST['limit']} OFFSET {$_POST['start']}");
} 
